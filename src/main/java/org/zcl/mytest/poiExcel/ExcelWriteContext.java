...
//	private Map <String ,CellStyle> cellStyles=new HashMap<String ,CellStyle>();
	...
	public Font getFont(String fontName, short size, boolean bold, boolean italic, boolean underLine, boolean deleteLine){
	...
	private Font findExistedFont(String fontName, short size, boolean bold, boolean italic){
		for (Font _font:fonts){
			if (_font.getFontHeightInPoints()==size && _font.getStrikeout()==bold && _font.getItalic()==italic){
				return _font;
			}
		}
		return null;
	...
	private Font createFont(String fontName, short size, boolean bold, boolean italic, boolean underLine, boolean deleteLine){
	...
	private Cell getCell(String workSheetName, int row, int column){
		...
		if (result==null) result=workbook.getSheet(workSheetName).getRow(row).createCell(column);
		return result; 
	}
}